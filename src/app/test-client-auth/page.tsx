      ...
      try {
        ...
        if (token) {
          ...
          setTokenInfo({
            hasToken: false,
            error:
              "JWT template 'convex' returned null - template might not exist!",
          });
        }
      } catch (e) {
        setTokenInfo({
  ...
  return (
    <div className="p-8 space-y-4 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold">
        Client-Side Auth Test (Outside Dashboard)
      </h1>
      ...
      <div className="p-4 bg-purple-50 border border-purple-200 rounded">
        <h2 className="font-semibold text-purple-800">
          JWT Token Info (template: &apos;convex&apos;):
        </h2>
        {tokenInfo === null ? (
          ...
          <>
            ...
            <p>
              Issuer (iss):{" "}
              <code className="bg-white px-1">{tokenInfo.issuer}</code>
            </p>
            <p>
              Audience (aud):{" "}
              <code className="bg-white px-1">{tokenInfo.audience}</code>
            </p>
            <p className="mt-2 text-sm">
              Expected in Convex auth.config:
              domain=&quot;https://clerk.ortoclub.com&quot;,
              applicationID=&quot;convex&quot;
            </p>
            <p
              className={
                tokenInfo.issuer === "https://clerk.ortoclub.com"
                  ? "text-green-600"
                  : "text-red-600"
              }
            >
            ...
            <p
              className={
                tokenInfo.audience === "convex"
                  ? "text-green-600"
                  : "text-red-600"
              }
            >
              Audience match:{" "}
              {tokenInfo.audience === "convex" ? "✅ YES" : "❌ NO"}
            </p>
          </>
        ) : (
      ...
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded">
        ...
        <p>
          ...
          <code className="bg-white px-1 text-xs">
      ...
      <div className="p-4 bg-green-50 border border-green-200 rounded">
        <h2 className="font-semibold text-green-800">
          Convex Query Result (api.users.current):
        </h2>
        {currentUser === undefined ? (
          <p className="text-yellow-600">Loading from Convex...</p>
        ) : currentUser === null ? (
          <p className="text-red-500">
            ❌ User not found in Convex (returned null) - Check console for auth
            errors!
          </p>
        ) : (
          <>
