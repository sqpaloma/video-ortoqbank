...
export const createPendingOrder = mutation({
  ...
  handler: async (ctx, args): Promise<{
    ...
    if (args.couponCode && args.couponCode.trim()) {
      ...
      const couponResult = await ctx.runQuery(api.promoCoupons.validateAndApplyCoupon, {
        code: args.couponCode,
        originalPrice: basePrice,
        userCpf: args.cpf.replaceAll(/\D/g, ''),
      });

      if (couponResult.isValid && couponResult.finalPrice !== undefined && couponResult.discountAmount !== undefined) {
    ...
    const pendingOrderId: Id<"pendingOrders"> = await ctx.db.insert('pendingOrders', {
      email: args.email,
